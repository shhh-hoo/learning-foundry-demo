...
const scenes: readonly StoryScene[] = [
  ...
  { id: 3, title: "Pattern threshold", persona: "Teacher · Learning Product Owner", time: "Three days later", route: "?view=studio&section=patterns&embedded=1", required: ["PATTERN_THRESHOLD_REACHED"], proves: "Two historical traces plus the current conversation create the third matching evidence record.", initialExplanation: "The repository is re-aggregated from persisted evidence before the pattern appears." },
  { id: 4, title: "Candidate and governance", persona: "Teacher · Subject Expert", time: "Three days later", route: "?view=studio&section=candidate&embedded=1", required: ["CANDIDATE_CREATED", "CANDIDATE_EVALUATED", "COMPONENT_APPROVED"], proves: "Evidence proposes a change; evaluation and expert review authorize it.", initialExplanation: "Create the candidate, run evaluation, then record expert approval inside Foundry Studio." },
...
const eventCopy: Partial<Record<DemoEventType, { readonly title: string; readonly body: string; readonly evidence: string }>> = {
  ...
  CANDIDATE_EVALUATED: { title: "Evaluation passed", body: "Foundry checks the draft contract before a human can approve it.", evidence: "15 governed checks" },
