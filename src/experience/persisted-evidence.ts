...
interface PersistedRun {
  ...
  readonly request: { readonly conversationId: string; readonly inputOrigin: InputOrigin }; readonly prompt: { readonly version: string }; readonly capabilityRegistry: { readonly version: string };
...
export async function loadPersistedLearningEvidence(fetcher: typeof fetch = fetch): Promise<PersistedLearningEvidence> {
  const [agentResponse, diagnosisResponse] = await Promise.all([fetcher("http://127.0.0.1:4176/agent/runs"), fetcher("http://127.0.0.1:4177/diagnoses")]);
  ...
  const agentTraces: AgentTrace[] = completedRuns.map((run) => ({
    traceId: run.traceId, conversationId: run.request.conversationId, inputOrigin: run.request.inputOrigin, provider: run.provider, model: run.model, thinkingMode: run.thinkingMode,
  ...
  const diagnoses = (diagnosisBody.diagnoses ?? []).flatMap((record): LearnerDiagnosisRecord[] => {
    const run = agentForDiagnosis.get(record.traceId); if (!run) return [];
    return [{ traceId: record.traceId, agentTraceId: run.traceId, inputOrigin: run.request.inputOrigin, origin: "TOOL_OUTPUT", componentId: record.component.id, componentVersion: record.component.version, decision: record.diagnosis.decision, firstPedagogicalIssue: record.diagnosis.firstPedagogicalIssue ?? null, failureCode: record.diagnosis.failureCode ?? null, evidence: record.diagnosis.evidence ?? [], recommendedSupport: record.recommendedSupport, createdAt: record.timestamp }];
  });
