...
import { executeAssetStage, type AssetRuntimeDependencies } from "@/application/asset-runtime";
...
export function buildAssetRuntimeGraph(
  ...
  return new StateGraph(AssetRuntimeState)
    .addNode("run_exact_asset_stage", async (state) => {
      const result = await executeAssetStage(state.actor, {
        taskId: state.taskId,
      ...
      return {
        ...
        runtimeStatus: result.delivery.status as "SUCCEEDED",
      };
    })
