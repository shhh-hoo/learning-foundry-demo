...
export function buildLearnerTaskGraph(checkpointer?: BaseCheckpointSaver) {
  const explanation = buildExplanationGraph();
  const diagnosis = buildDiagnosisGraph();
  return new StateGraph(LearnerTaskState)
    .addNode("record_learner_input", async (state) => {
      await appendConversationEvent(state.actor, { taskId: state.taskId, episodeId: state.episodeId, kind: "MESSAGE", content: state.message, actorType: "LEARNER" });
      return {};
    })
    .addNode("classify_product_action", (state) => ({ action: classifyProductAction(state) }))
    .addNode("explanation_subgraph", async (state) => {
      const result = await explanation.invoke({ actor: state.actor, taskId: state.taskId, episodeId: state.episodeId, question: state.message });
      return { result: { response: result.response, citations: result.citations, responseEventId: result.responseEventId } };
    })
