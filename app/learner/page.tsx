...
import { AttemptForm, CloseTaskButton, CreateTaskForm, MessageForm, RetryAttemptForm } from "@/components/ClientActions";
...
export default async function LearnerPage({ searchParams }: { searchParams: Promise<{ task?: string }> }) {
  ...
  return <>
    ...
    <div className="showcase-banner"><Badge tone="warn">Synthetic showcase data</Badge> External model synthesis, mature hybrid retrieval, multimodal retrieval and Standard Trainer are unavailable in this checkpoint.</div>
    <div className="workspace-grid">
      ...
      <div className="stack wide-column">{detail && activeEpisode ? <>
        ...
        <Card eyebrow="Context Compiler" title="Scoped Context">{detail.contexts[0] ? <><p>Compiler {detail.contexts[0].compilerVersion} · lifecycle selection only</p><div className="header-actions"><Badge tone="warn">Token budgeting · NOT_ENFORCED</Badge><Badge tone="warn">Modality budgeting · UNAVAILABLE</Badge></div><p>Selected and excluded items reflect lifecycle filtering only. Stored budget fields are not applied as model-token or modality limits.</p><pre>{JSON.stringify({ selected: detail.contexts[0].selectedItems, excluded: detail.contexts[0].excludedItems }, null, 2)}</pre></> : <Empty>No Context compilation has run for this Task.</Empty>}</Card>
        <Card eyebrow="Authorized delivery" title="Authorized Evidence catalog"><p>These LEARNING-purpose sources are authorized for this Task/course. They are not necessarily used by an answer; answer-level sourceRefs and evidenceRefs appear with each conversation event.</p><p><Badge tone="warn">PostgreSQL lexical candidate retrieval</Badge> No embedding or reranker claim is made.</p>{evidenceCatalog.map(({ evidence, source }) => <article className="evidence-card" key={evidence.id}><strong>{evidence.title}</strong><p>{evidence.content}</p>{evidence.structuredContent ? <pre>{JSON.stringify(evidence.structuredContent, null, 2)}</pre> : null}<small>{source.title} · v{source.version} · {evidence.locator} · {source.rights}</small></article>)}{evidenceCatalog.length === 0 ? <Empty>No Evidence has explicit authorized LEARNING delivery and course/Reference Pack alignment for this Task.</Empty> : null}</Card>
        <Card eyebrow="Attempt" title="Capture learner reasoning"><AttemptForm taskId={detail.task.id} episodeId={activeEpisode.id}/>{detail.attempts.map((attempt) => <article className="list-row" key={attempt.id}><span><strong>{attempt.prompt}</strong><small>{attempt.response}</small></span><Badge tone="warn">{detail.observations.find((item) => item.attemptId === attempt.id)?.status ?? "REVIEW_PENDING"}</Badge></article>)}</Card>
