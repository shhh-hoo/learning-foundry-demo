import { z } from "zod";
import { createAssetOptimizationProposal } from "@/application/asset-optimization";
...
export async function POST(request: Request) {
  try {
    return await withApiActor(async (actor) => Response.json(
      await createAssetOptimizationProposal(actor, Proposal.parse(await request.json())),
      { status: 201 },
    ));
