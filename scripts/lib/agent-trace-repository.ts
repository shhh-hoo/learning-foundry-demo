...
export class FileAgentTraceStore implements AgentTraceStore {
  ...
  async fail(traceId: string, terminalError: { readonly code: string; readonly message: string }, completedAt: string, observability: Parameters<AgentTraceStore["fail"]>[3] = {}): Promise<void> { await this.mutate(traceId, (record) => ({ ...record, status: "FAILED", terminalError: sanitize(terminalError) as typeof terminalError, ...observability, completedAt, updatedAt: completedAt })); }
