...
const trace = (index: number, conversationId: string): AgentTrace => ({ traceId: `agent-trace-${index}`, conversationId, inputOrigin: "PRESET_INPUT", runPurpose: "PRODUCT", provider: "deepseek", model: "configured-model", thinkingMode: "disabled", promptVersion: "1.3.0", capabilityRegistryVersion: "1.0.0", startedAt: `2026-07-16T10:0${index}:00.000Z`, completedAt: `2026-07-16T10:0${index}:01.000Z`, toolCalls: [{ name: "run_learner_diagnosis", arguments: {}, resultRef: `result-${index}`, status: "SUCCEEDED" }], finalResponse: { status: "ANSWERED", learnerMessage: "Check the coefficient ratio.", sourceRefs: [], diagnosisTraceId: `trainer-trace-${index}` }, latencyMs: 1000 });
...
describe("actual-run causality", () => {
  it("starts with no diagnosis evidence and reaches the pattern threshold only after three actual runs", () => {
    ...
    state = applyAgentRun(state, "run 1", trace(1, state.conversationId), result(1));
    state = applyAgentRun(state, "run 2", trace(2, state.conversationId), result(2));
    expect(aggregatePatternEvidence(state.diagnoses).thresholdReached).toBe(false);
    state = applyAgentRun(state, "run 3", trace(3, state.conversationId), result(3));
