...
const trace = (index: number): AgentTrace => ({ traceId: `agent-trace-${index}`, conversationId: "test-conversation", inputOrigin: "PRESET_INPUT", runPurpose: "PRODUCT", provider: "deepseek", model: "configured-model", thinkingMode: "disabled", promptVersion: "1.0.0", capabilityRegistryVersion: "1.0.0", startedAt: `2026-07-16T10:0${index}:00.000Z`, completedAt: `2026-07-16T10:0${index}:01.000Z`, toolCalls: [{ name: "run_learner_diagnosis", arguments: {}, resultRef: `result-${index}`, status: "SUCCEEDED" }], finalResponse: { status: "ANSWERED", learnerMessage: "Check the coefficient ratio.", sourceRefs: [], diagnosisTraceId: `trainer-trace-${index}` }, latencyMs: 1000 });
