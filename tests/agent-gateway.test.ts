...
describe("DeepSeek Agent Gateway", () => {
  ...
  it("passes evalCaseId only for AgentEval transport metadata", async () => {
    ...
    const trace = { traceId: "agent-eval-trace", conversationId: "eval", inputOrigin: "USER_INPUT", runPurpose: "AGENT_EVAL", provider: "deepseek", model: "configured", thinkingMode: "disabled", promptVersion: "1", capabilityRegistryVersion: "1", startedAt: "2026-07-16T10:00:00.000Z", completedAt: "2026-07-16T10:00:01.000Z", toolCalls: [], finalResponse: { status: "ANSWERED", learnerMessage: "Hello", sourceRefs: [] }, latencyMs: 1000 } satisfies AgentTrace;
    ...
    const valid = await gateway.handle(new Request("http://127.0.0.1:4176/agent/runs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ conversationId: "eval", inputOrigin: "USER_INPUT", runPurpose: "AGENT_EVAL", evalCaseId: "case-1", messages: [{ role: "user", content: "hello" }] }) }));
