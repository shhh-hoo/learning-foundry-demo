...
/** Class B: evidence-bound suggestion to review one exact recorded route and its policy. */
export const routingOptimizationProposals = product.table("routing_optimization_proposals", {
  id: id(),
  institutionId: uuid("institution_id").notNull().references(() => institutions.id, { onDelete: "cascade" }),
  courseId: uuid("course_id").notNull().references(() => courses.id, { onDelete: "cascade" }),
  taskId: uuid("task_id").notNull().references(() => learningTasks.id, { onDelete: "cascade" }),
  ...
  diagnosticObservationId: uuid("diagnostic_observation_id").notNull().references(() => diagnosticObservations.id),
  capabilityResolutionId: uuid("capability_resolution_id").notNull().references(() => capabilityResolutions.id),
  capabilityResolutionInputHash: text("capability_resolution_input_hash").notNull(),
  selectedCapabilityId: uuid("selected_capability_id").notNull().references(() => capabilities.id),
  selectedCapabilityVersionId: uuid("selected_capability_version_id").notNull().references(() => capabilityVersions.id),
  selectedCapabilityVersionContentHash: text("selected_capability_version_content_hash").notNull(),
  activityPlanId: uuid("activity_plan_id").notNull().references(() => activityPlans.id),
  runtimeDeliveryId: uuid("runtime_delivery_id").notNull().references(() => runtimeDeliveries.id),
  learnerAttemptId: uuid("learner_attempt_id").notNull().references(() => learnerAttempts.id),
  learnerAttemptContentHash: text("learner_attempt_content_hash").notNull(),
  teacherInterventionId: uuid("teacher_intervention_id").notNull().references(() => teacherInterventions.id),
  proposalType: text("proposal_type").notNull(),
  signalKind: text("signal_kind").notNull(),
  rationale: text("rationale").notNull(),
  proposedChange: jsonb("proposed_change").$type<Record<string, unknown>>().notNull(),
  ...
  uniqueIndex("routing_optimization_intervention_uq").on(table.teacherInterventionId),
  uniqueIndex("routing_optimization_request_hash_uq").on(table.institutionId, table.requestHash),
  index("routing_optimization_course_idx").on(table.institutionId, table.courseId, table.createdAt),
  check("routing_optimization_type_ck", sql`${table.proposalType} = 'ROUTING' AND ${table.signalKind} = 'TEACHER_EXCLUSION_OVERRIDE'`),
  check("routing_optimization_state_ck", sql`${table.state} = 'PENDING_GOVERNANCE'`),
  check("routing_optimization_confidence_ck", sql`${table.confidence} >= 0 AND ${table.confidence} <= 1`),
  check("routing_optimization_json_ck", sql`jsonb_typeof(${table.proposedChange})='object' AND jsonb_typeof(${table.evidenceSnapshot})='object' AND jsonb_typeof(${table.evidenceRefs})='array' AND jsonb_typeof(${table.limitations})='array'`),
  check("routing_optimization_hash_ck", sql`length(${table.contextSnapshotHash})>7 AND length(${table.capabilityResolutionInputHash})>7 AND length(${table.selectedCapabilityVersionContentHash})>7 AND length(${table.learnerAttemptContentHash})>7 AND length(${table.evidenceHash})>7 AND length(${table.requestHash})>7`),
]);

/** Class A: append-only human governance of a Routing Optimization Proposal's next action. */
export const routingOptimizationDecisions = product.table("routing_optimization_decisions", {
  id: id(),
  institutionId: uuid("institution_id").notNull().references(() => institutions.id, { onDelete: "cascade" }),
  courseId: uuid("course_id").notNull().references(() => courses.id, { onDelete: "cascade" }),
  proposalId: uuid("proposal_id").notNull().references(() => routingOptimizationProposals.id),
  taskId: uuid("task_id").notNull().references(() => learningTasks.id),
  capabilityResolutionId: uuid("capability_resolution_id").notNull().references(() => capabilityResolutions.id),
  action: text("action").notNull(),
  rationale: text("rationale").notNull(),
  ...
  uniqueIndex("routing_optimization_decision_proposal_uq").on(table.proposalId),
  uniqueIndex("routing_optimization_decision_actor_key_uq").on(table.institutionId, table.decidedBy, table.idempotencyKey),
  check("routing_optimization_decision_action_ck", sql`${table.action} IN ('REQUEST_POLICY_REVIEW','KEEP_CURRENT_POLICY')`),
  check("routing_optimization_decision_payload_ck", sql`length(btrim(${table.rationale}))>=5 AND length(${table.requestHash})>7`),
]);
